...
def filter_images(**kwargs):
    ...
    if kwargs.get('extract_poses', False):
        ...
        for i in image_paths:
            target_path = os.path.join(
                kwargs['out_dir'], 'uncropped', os.path.basename(i))
            if not os.path.exists(target_path):
                shutil.copy(i, target_path)
        image_paths = subdivide_images_with_openpose(
    ...
    if meta_missing:
        ...
        if len(meta_missing) > 10:
            print(' ...', len(meta_missing)-10, 'more')
...
def write_metadata(metadata, **kwargs):
    ...
    # create the map from values to images with that value (if values present)
    values_d = defaultdict(list)
    for i in metadata:
        value = i.get('value', '')
        if value:
            values_d[value].append(clean_filename(i['filename']))
    # find the min and max values to show on the value slider
    values = np.array([int(i.strip()) for i in values_d if is_number(i)])
    v_domain = {'min': float('inf'), 'max': -float('inf')}
    v_mean = np.mean(values)
    v_std = np.std(values)
    for i in values:
        # update the values domain with all non-outlier values
        if abs(v_mean-i) < (v_std*4):
            v_domain['min'] = int(min(i, v_domain['min']))
            v_domain['max'] = int(max(i, v_domain['max']))
    # write the values json
    if len(values_d) > 1:
        write_json(os.path.join(out_dir, 'values.json'), {
            'domain': v_domain,
            'values': values_d,
        }, **kwargs)
...
def get_inception_vectors(**kwargs):
    ...
    print(
        ' * generating Inception vectors for {} images'.format(len(kwargs['image_paths'])))
...
def get_pose_layout(**kwargs):
    ...
    if not kwargs.get('extract_poses', False):
        return False
    ...
    for i in stream_images(**kwargs):
        ...
        # slice out the confidence scores
        vec = normalize_2d_vector(vec[:, :2])
...
def subdivide_images_with_openpose(**kwargs):
    ...
    parsed_path = os.path.join(
        kwargs['out_dir'], 'image-vectors', 'openpose', 'parsed.json')
    vectors_path = os.path.join(
        kwargs['out_dir'], 'image-vectors', 'openpose', 'vectors.json')
    # load d[original_image_path] = [{daughter_image_path: [vert confidence array]}]
    parsed_dict = json.load(open(parsed_path)) if os.path.exists(
        parsed_path) else None
    if not parsed_dict:
        parsed_dict = defaultdict(lambda: defaultdict())
    ...
    graph_path = os.path.join(
        dirname(realpath(__file__)), 'models', 'graph', 'cmu', 'graph_opt.pb')
    ...
    for idx, i in enumerate(stream_images(image_paths=kwargs['image_paths'])):
        ...
        humans = estimator.inference(
            im, resize_to_default=True, upsample_size=4.0)
        for hdx, human in enumerate(humans):
            # add image to cache (loaded json does not autovivify)
            if file_basename not in parsed_dict:
                parsed_dict[cache_filename] = {}
...
def pose_vector_is_valid(vector, **kwargs):
    ...
    if len(confidence_vals) < kwargs['min_vertices']:
        return False
    if min(confidence_vals) < kwargs['min_score']:
        return False
    return True
...
def get_categorical_layout(null_category='Other', margin=2, **kwargs):
    ...
    if not any(categories) or len(set(categories)) == 1:
        return False
    d = defaultdict(list)
...
def parse():
    ...
    parser.add_argument('--min_distance', type=float,
                        default=config['min_distance'], help='the min_distance argument for umap')
    parser.add_argument(
    ...
    parser.add_argument('--extract_poses', action='store_true',
                        help='create pose-based embeddings of input images')
    ...
    parser.add_argument('--min_score', type=float,
                        default=config['min_score'], help='min vertex score in a pose-based embedding')
    parser.add_argument('--min_vertices', type=int,
                        default=config['min_vertices'], help='min number of discovered vertices in a pose-based embedding 0:18')
